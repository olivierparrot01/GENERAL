...
with st.sidebar.expander("Informations"):
    st.sidebar.info('''Les temps de réponse sont un peu longs, il faut patienter...Mais une fois que la carte est affichée, la navigation est fluide.
                    Les pts Gun sont classés en 57 secteurs géographiques (secteur d'accumulation) : ces secteurs sont tels que chaque point a un voisin distant d'au plus 20 km ; cette approche permet de visualiser sur la carte les pts groupés, ce qui est efficace dans la vérification des coordonnées.
...
selected_codes1 = filtered_df[filtered_df['Secteur'].isin(selected_cat)]['Code_AIOT'].tolist()
...
filtered_data = pd.concat([filtered_df[filtered_df['Code_AIOT'].isin(selected_codes)], filtered_df[filtered_df['Secteur'].isin(selected_cat)]])
